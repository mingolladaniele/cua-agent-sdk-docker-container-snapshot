...
from snapshot_manager.models import SnapshotConfig, SnapshotTrigger, SnapshotStatus
...
class TestSnapshotManager:
    ...
    async def test_list_snapshots(self, snapshot_manager):
        ...
        container_snapshots = await snapshot_manager.list_snapshots(container_id="test_container_id")
    ...
    async def test_cleanup_old_snapshots(self, snapshot_manager):
        ...
        with patch('snapshot_manager.models.datetime') as mock_datetime:
            ...
            old_metadata = await snapshot_manager.create_snapshot("test_container", SnapshotTrigger.MANUAL)
            old_metadata.timestamp = old_time
            await snapshot_manager.storage.update_metadata(old_metadata)
        
        # Create a recent snapshot
        recent_metadata = await snapshot_manager.create_snapshot("test_container", SnapshotTrigger.MANUAL)
