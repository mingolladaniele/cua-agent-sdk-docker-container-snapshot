...
from .interfaces import SnapshotProvider, SnapshotStorage, SnapshotError
from .models import (
    SnapshotMetadata, 
    SnapshotConfig, 
    SnapshotTrigger, 
...
class SnapshotManager:
    ...
    async def restore_snapshot(
        ...
        if metadata.status != SnapshotStatus.COMPLETED:
            raise SnapshotError(f"Snapshot {snapshot_id} is not in completed state (status: {metadata.status})")
        ...
        try:
            ...
            logger.info(f"Successfully restored container {container_id} from snapshot {snapshot_id}")
            return container_id
    ...
    async def delete_snapshot(self, snapshot_id: str) -> None:
        ...
        metadata = await self.storage.load_metadata(snapshot_id)
        if not metadata:
            logger.warning(f"Snapshot {snapshot_id} metadata not found, but proceeding with cleanup")
            return
    ...
    async def _enforce_snapshot_limits(self, container_id: str) -> None:
        ...
        if len(container_snapshots) >= self.config.max_snapshots_per_container:
            ...
            for snapshot in snapshots_to_delete:
                try:
                    ...
                    logger.info(f"Auto-deleted old snapshot {snapshot.snapshot_id} due to per-container limit")
                except Exception as e:
        ...
        if len(all_snapshots) >= self.config.max_total_snapshots:
            ...
            for snapshot in snapshots_to_delete:
                try:
                    await self.delete_snapshot(snapshot.snapshot_id)
                    logger.info(f"Auto-deleted old snapshot {snapshot.snapshot_id} due to total limit")
                except Exception as e:
                    logger.error(f"Failed to auto-delete snapshot {snapshot.snapshot_id}: {e}")
        ...
        if stats["total_size_gb"] > self.config.max_storage_size_gb:
            ...
            for snapshot in reversed(all_snapshots):  # Oldest first
                try:
                    await self.delete_snapshot(snapshot.snapshot_id)
                    logger.info(f"Auto-deleted snapshot {snapshot.snapshot_id} due to storage size limit")
