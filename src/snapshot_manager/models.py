...
class SnapshotMetadata(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.now, description="When the snapshot was created")
    
    # Context information
    trigger: SnapshotTrigger = Field(..., description="What triggered this snapshot")
    action_context: Optional[str] = Field(None, description="Context of the action that triggered the snapshot")
    run_id: Optional[str] = Field(None, description="ID of the agent run this snapshot belongs to")
    
    # Status and lifecycle
    status: SnapshotStatus = Field(default=SnapshotStatus.CREATING, description="Current status of the snapshot")
    ...
    parent_snapshot_id: Optional[str] = Field(None, description="ID of parent snapshot if this is a restored state")
    restoration_count: int = Field(default=0, description="Number of times this snapshot has been restored")
...
class SnapshotConfig(BaseModel):
    ...
    max_snapshots_per_container: int = Field(default=10, description="Maximum snapshots to keep per container")
    max_total_snapshots: int = Field(default=100, description="Maximum total snapshots across all containers")
    max_storage_size_gb: float = Field(default=10.0, description="Maximum storage size in GB")
    
    # Cleanup configuration
    cleanup_on_exit: bool = Field(default=True, description="Whether to clean up old snapshots on exit")
    auto_cleanup_days: int = Field(default=7, description="Auto-delete snapshots older than this many days")
    
    # Performance configuration
    compression_enabled: bool = Field(default=True, description="Whether to compress snapshots")
    parallel_operations: bool = Field(default=True, description="Whether to allow parallel snapshot operations")
    
    # Integration configuration
    include_volumes: bool = Field(default=True, description="Whether to include mounted volumes in snapshots")
    ...
    naming_pattern: str = Field(
        default="{container_name}_{trigger}_{timestamp}",
        description="Pattern for snapshot naming"
    )
...
class RestoreOptions(BaseModel):
    ...
    preserve_networks: bool = Field(default=True, description="Whether to preserve network connections")
    ...
    port_mappings: Dict[str, str] = Field(
        default_factory=dict,
        description="Port mappings to override (container_port: host_port)"
    )
