...
from .models import SnapshotConfig, SnapshotTrigger, RestoreOptions
...
@click.option('--trigger', type=click.Choice([t.value for t in SnapshotTrigger]), 
              default='manual', help='Snapshot trigger type')
...
def create(ctx, container_id, trigger, description, context, run_id):
    ...
    async def _create():
        ...
        try:
            ...
            click.echo(f"   Size: {metadata.size_bytes / (1024*1024):.1f} MB" if metadata.size_bytes else "   Size: Unknown")
        ...
        try:
            ...
            if json_output:
                ...
                for snapshot in snapshots:
                    ...
                    click.echo(f"   Container: {snapshot.container_name} ({snapshot.container_id[:12]})")
...
@click.option('--preserve-networks/--no-preserve-networks', default=True, 
              help='Preserve network connections')
@click.option('--preserve-volumes/--no-preserve-volumes', default=True,
              help='Preserve volume mounts')
@click.pass_context
    ...
    async def _delete():
        ...
        try:
            if not force:
                ...
                if metadata:
                    click.echo(f"About to delete snapshot:")
