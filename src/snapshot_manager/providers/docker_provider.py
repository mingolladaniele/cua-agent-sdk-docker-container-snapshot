...
from ..interfaces import SnapshotProvider, SnapshotError, ContainerNotFoundError
from ..models import SnapshotMetadata, RestoreOptions, SnapshotStatus
...
class DockerSnapshotProvider(SnapshotProvider):
    ...
    def _get_container_info(self, container_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "network_settings": container.attrs["NetworkSettings"] if "NetworkSettings" in container.attrs else {}
            }
        except NotFound:
    ...
    async def validate_container(self, container_id: str) -> bool:
        ...
        try:
            ...
            if container_info["status"] not in valid_statuses:
                logger.warning(f"Container {container_id} has status '{container_info['status']}', may not be snapshotable")
                return False
    ...
    async def create_snapshot(
        ...
        try:
            ...
            logger.info(f"Successfully created snapshot {metadata.snapshot_id} for container {container_id}")
            return metadata
    ...
    async def restore_snapshot(
        ...
        try:
            ...
            container_name = options.new_container_name or f"{metadata.container_name}_restored_{int(datetime.now().timestamp())}"
            ...
            original_network = metadata.agent_metadata.get("original_network_settings", {})
            ...
            if options.port_mappings:
                ...
                run_config["ports"] = {port: None for port in original_config["ExposedPorts"].keys()}
            ...
            logger.info(f"Creating restored container '{container_name}' from image {metadata.image_id}")
            ...
            logger.info(f"Successfully restored container {container.id} from snapshot {metadata.snapshot_id}")
            return container.id
